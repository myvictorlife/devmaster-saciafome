...
var PromotionComponent = (function () {
    ...
    PromotionComponent.prototype.editPromotion = function (content, promotion) {
        var coupon = promotion.cupom ? JSON.parse(promotion.cupom) : { name: '', discount_type: '', price: '' };
        var name = JSON.parse(coupon.name).name;
    ...
    PromotionComponent.prototype.insertOrUpdate = function () {
        ...
        if (this.formulario.valid) {
            if (this.formulario.value.id) {
                var json = this.formulario.value;
                json.cupom = JSON.stringify(this.mountCoupon(json));
                this.promotionService.update(json)
                    .subscribe(function (result) {
                    if (result.status) {
                ...
                var json = this.formulario.value;
                json.cupom = JSON.stringify(this.mountCoupon(json));
                this.promotionService.register(json)
                    .subscribe(function (result) {
                    if (result.status) {
                        _this.promotions = _this.promotionService.getAll();
                        _this.toastr.success(result.message, '');
    ...
    PromotionComponent.prototype.mountCoupon = function (obj) {
        return {
            name: obj.cupom,
            price: obj.price,
